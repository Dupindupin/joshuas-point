...
export async function generateMetadata({params}: PremiumJourneyPageProps): Promise<Metadata> {
  ...
  const journey = await getPremiumGuidePreviewJourney(slug)
  ...
  return {
    ...
    title: `${journey.title} | Premium Guide Preview`,
  }
}
...
export default async function PremiumJourneyPage({params}: PremiumJourneyPageProps) {
  ...
  const [journey, journeys] = await Promise.all([
    getPremiumGuidePreviewJourney(slug),
    getPremiumGuidePreviewJourneys(),
  ])
  ...
  return (
    <>
      ...
      <main className="bg-canvas pt-24">
        ...
        <SectionSpacing
          aria-label="Move between Premium Guide journeys"
            ...
            <div className="grid gap-10 sm:grid-cols-2 sm:gap-12">
              <div>
                {previousJourney ? (
                  <Link
                    ...
                    href={`/premium-guide-preview/journeys/${previousJourney.slug}`}
                  >
                    <EditorialText variant="eyebrow">← Previous journey</EditorialText>
                    <p className="mt-4 font-display text-3xl text-ink group-hover:text-accent">
                      {previousJourney.title}
                    </p>
                  </Link>
                ) : null}
              </div>
              <div className="sm:text-right">
                {nextJourney ? (
                  <Link
                    className="group block rounded-sm focus-visible:outline-2 focus-visible:outline-offset-6 focus-visible:outline-focus"
                    href={`/premium-guide-preview/journeys/${nextJourney.slug}`}
                  >
