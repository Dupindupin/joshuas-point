...
function confirmationPage(token: string) {
  ...
  return new NextResponse(
    `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><title>Confirm updates — Joshua's Point</title><style>:root{color-scheme:light dark}*{box-sizing:border-box}body{margin:0;min-height:100vh;display:grid;place-items:center;padding:2rem;background:#f3ede6;color:#282828;font-family:Georgia,'Times New Roman',serif}.card{width:min(100%,36rem);padding:clamp(2rem,7vw,4rem);border:1px solid #d8cec0;background:#faf7f2;text-align:center}img{display:block;width:min(100%,18rem);height:auto;margin:0 auto 2rem}h1{margin:0 0 1rem;color:#1f3d3a;font-size:clamp(2rem,7vw,3.3rem);font-weight:400;line-height:1.05}p{line-height:1.7}button{margin-top:1rem;border:0;border-radius:999px;background:#1f3d3a;color:#f3ede6;cursor:pointer;font:700 .8rem Arial,sans-serif;letter-spacing:.08em;padding:.9rem 1.4rem;text-transform:uppercase}button:focus-visible{outline:3px solid #c8a26a;outline-offset:4px}@media(prefers-color-scheme:dark){body{background:#191c1a;color:#f3ede6}.card{background:#202420;border-color:#496b5b}h1{color:#f3ede6}}</style></head><body><main class="card"><img src="/brand/logo-horizontal.png" alt="Joshua's Point"><h1>Confirm your email</h1><p>Choose confirm to receive occasional news from Joshua’s Point.</p><form action="/api/subscriptions/confirm" method="post"><input type="hidden" name="token" value="${escapedToken}"><button type="submit">Confirm my email</button></form></main></body></html>`,
    {
      headers: {
