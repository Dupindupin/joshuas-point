...
const providerLabels: Record<MapProviderName, string> = {
...
export function EditorialMapPlaceholder({ariaLabel, data, provider}: EditorialMapPlaceholderProps) {
  ...
  const status = provider
    ? `${providerLabels[provider]} adapter reserved for future integration`
    : 'Interactive map reserved for a future provider'

  return (
    <div
      aria-label={`${ariaLabel}. ${status}.`}
      className="relative h-full min-h-[28rem] overflow-hidden bg-charcoal lg:min-h-[42rem]"
      role="img"
    >
      ...
      {pointCount > 0 ? (
        <div
          aria-hidden="true"
          className="absolute top-[42%] left-[63%] h-4 w-4 -translate-x-1/2 -translate-y-1/2 rounded-full border border-linen/80"
        >
          <span className="absolute top-1/2 left-1/2 h-px w-12 -translate-x-1/2 -translate-y-1/2 bg-linen/30" />
          <span className="absolute top-1/2 left-1/2 h-12 w-px -translate-x-1/2 -translate-y-1/2 bg-linen/30" />
        </div>
      ) : null}

      <div className="absolute right-6 bottom-6 left-6 flex items-end justify-between gap-8 sm:right-8 sm:bottom-8 sm:left-8">
        <p className="max-w-sm font-body text-[0.6875rem] font-semibold tracking-[0.2em] text-linen/48 uppercase">
          {status}
        </p>
        <p
          aria-hidden="true"
          className="hidden font-body text-[0.625rem] tracking-[0.12em] text-linen/30 uppercase sm:block"
        >
